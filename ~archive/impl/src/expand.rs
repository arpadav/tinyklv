...
impl From<ast::Input> for proc_macro::TokenStream {
    fn from(mut input: ast::Input) -> Self {
        // --------------------------------------------------
        ...
        let mut missing_decoders = Vec::new();
        for f in input.fattrs.iter() {
            all_encoders_exist &= f.contents.enc().is_some();
            all_decoders_exist &= f.contents.dec().is_some();
            f.contents.enc().is_none().then(|| { missing_encoders.push(f.name.to_string()) });
            f.contents.dec().is_none().then(|| { missing_decoders.push(f.name.to_string()) });
        }

        let mut expanded = quote! {};

        match (all_decoders_exist, input.sattr.allow_unimplemented_decode) {
            (true, _) => {
            ...
            (false, false) => panic!("\n{}", format!(
                "{}\nIf you want to allow for unimplemented decoders, set the `allow_unimplemented_decode` attribute on your struct.\nE.g. `#[klv(allow_unimplemented_decode)]`\n",
                missing_decoders
                    .iter()
                    .map(|name| format!("\t{}", crate::Error::MissingFunc(format!("field `{}`", name), "".into(), "dec".into(), "decoder".into())))
                    .collect::<Vec<_>>()
                    .join("\n")
            )),
            (false, true) => (),
        ...
        match (all_encoders_exist, input.sattr.allow_unimplemented_encode) {
            (true, _) => {
            ...
            (false, false) => panic!("\n{}", format!(
                "{}\nIf you want to allow for unimplemented encoders, set the `allow_unimplemented_encode` attribute on your struct.\nE.g. `#[klv(allow_unimplemented_encode)]`\n",
                missing_encoders
                    .iter()
                    .map(|name| format!("\t{}", crate::Error::MissingFunc(format!("field `{}`", name), "".into(), "enc".into(), "encoder".into())))
                    .collect::<Vec<_>>()
                    .join("\n")
            )),
            (false, true) => (),
...
fn gen_encode_impl(input: &ast::Input) -> proc_macro2::TokenStream {
    let name = &input.name;
    // --------------------------------------------------
    // default stream -> &[u8]
    // --------------------------------------------------
...
fn gen_items_encoded(input: &ast::Input, key_encoder: &PathLike, len_encoder: &PathLike) -> proc_macro2::TokenStream {
...
fn gen_decode_impl(input: &ast::Input) -> proc_macro2::TokenStream {
    let name = &input.name;
    // --------------------------------------------------
    // default stream -> &[u8]
    // --------------------------------------------------
    ...
    let seek_if_sentinel = match sentinel {
        Some(sentinel) => {
            ...
            quote! {
                ...
                pub(crate) static #sentinel_seeker_static_name: ::std::sync::LazyLock<::tinyklv::reexport::memchr::memmem::Finder> = ::std::sync::LazyLock::new(|| ::tinyklv::reexport::memchr::memmem::Finder::new(#sentinel));
                ...
                impl ::tinyklv::prelude::Seek<#stream> for #name {
                    ...
                    // ---- ^^^ ---- remember this is PACKET_LIFETIME_CHAR
                        ...
                        let input = match #sentinel_seeker_static_name.find(&input) {
                            ...
                            None => return Err(::tinyklv::reexport::winnow::error::ErrMode::Cut(
                        ...
                        let packet_len = match #len_decoder.parse_next(input) {
                            ...
                            Err(e) => return Err(e.backtrack().add_context(
                                ...
                                &checkpoint,
                                ::tinyklv::reexport::winnow::error::StrContext::Label(
                                    concat!("Found recognition sentinel, but could not parse packet length for `", stringify!(#name), "` packet")
                                )
                            )),
                        };
            ...
            fn decode(input: &mut #stream) -> ::tinyklv::reexport::winnow::PResult<Self> {
                ...
                loop {
                    match (
                        ...
                        Ok((key, len)) => {
                            let mut subinput = ::tinyklv::reexport::winnow::token::take::<usize, #stream, ::tinyklv::reexport::winnow::error::ContextError>(len).parse_next(input)?;
                            match key {
                                #items_match
                                _ => (),
                            }
...
fn gen_items_init(fatts: &Vec<ast::FieldAttrSchema>) -> proc_macro2::TokenStream {
    let field_initializations = fatts.iter().map(|field| {
        let ast::FieldAttrSchema { name, ty, .. } = field;
...
fn gen_items_match(fatts: &Vec<ast::FieldAttrSchema>) -> proc_macro2::TokenStream {
    let arms = fatts.iter().map(|field| {
        ...
        let dec = field.contents.dec().clone().unwrap_or_else(||
            panic!("{}", crate::Error::MissingFunc(format!("field `{}`", name), "".into(), "dec".into(), "decoder".into()))
        );
...
fn gen_item_set(struct_name: &syn::Ident, fatts: &Vec<ast::FieldAttrSchema>, elem_name_type_without_klv: Vec<(syn::Ident, syn::Type)>) -> proc_macro2::TokenStream {
