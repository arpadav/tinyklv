    ...
    pub fn from_field(input: &syn::Field) -> Option<Self> {
        ...
        let parsed: Option<MetaTuple> = input
            ...
            .filter(|attr| match attr.path().get_ident() {
                Some(ident) => ident.to_string() == crate::ATTR_NAME,
                None => false,
            })
            .next()
            .map(|attr| MetaTuple::from(format!("{}{}", crate::ATTR_NAME, attr.to_token_stream().to_string())));
        match parsed {
...
impl FieldAttrContents {
    ...
    fn set_enc(&mut self, enc: crate::ast::xcoder::PathLike) {
    ...
    pub fn enc(&self) -> Option<&crate::ast::xcoder::PathLike> {
        self.xcoder.get().map_or(None, |x| x.enc.as_ref())
    }

    fn set_dec(&mut self, dec: crate::ast::xcoder::PathLike) {
        match self.xcoder.get_mut() {
    ...
    pub fn dec(&self) -> Option<&crate::ast::xcoder::PathLike> {
