//! quick-protobuf x nested record: the embedded coordinate is an `Option<GpsCoord>`, the
...
/// [`QuickProtobuf`] implementation of [`Codec`] for [`Platform`]
impl Codec<Platform> for QuickProtobuf {
    fn encode(rec: &Platform) -> Vec<u8> {
        // --------------------------------------------------
        // build the proto message: embedded coord + repeated sensors + widened scalars
        // --------------------------------------------------
        let msg = generated::Platform {
            id: rec.id,
    ...
    fn decode(body: &[u8]) -> Option<Platform> {
        ...
        let m = generated::Platform::from_reader(&mut reader, body).ok()?;
        let coord = m.coord?;
        // --------------------------------------------------
