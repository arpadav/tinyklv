//! quick-protobuf x native record: whole-struct conversion. quick-protobuf has no std/chrono
//! conversions, so the temporal fields are raw integers (`native_raw.proto`) and every native
...
impl From<&NativeNested> for generated::NativeNested {
    fn from(rec: &NativeNested) -> Self {
        generated::NativeNested {
            ...
            timestamp_nanos: rec.timestamp.timestamp_nanos_opt().expect("timestamp in i64-ns range"),
...
/// [`QuickProtobuf`] implementation of [`Codec`] for [`NativeNested`]
impl Codec<NativeNested> for QuickProtobuf {
    fn encode(rec: &NativeNested) -> Vec<u8> {
        let msg = generated::NativeNested::from(rec);
        let mut out = Vec::new();
        {
    ...
    fn decode(body: &[u8]) -> Option<NativeNested> {
        ...
        generated::NativeNested::from_reader(&mut reader, body)
            .ok()?
            .try_into()
            .ok()
    }
