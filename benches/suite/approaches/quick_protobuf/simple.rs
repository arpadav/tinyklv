//! quick-protobuf x simple record: build the generated `Simple`, write it through a
...
/// [`QuickProtobuf`] implementation of [`Codec`] for [`Simple`]
impl Codec<Simple> for QuickProtobuf {
    fn encode(rec: &Simple) -> Vec<u8> {
        // --------------------------------------------------
        // widen KLV scalars into the proto message
        // --------------------------------------------------
        let msg = generated::Simple {
            a: rec.a.into(),
            b: rec.b.into(),
            c: rec.c,
            d: rec.d,
    ...
    fn decode(body: &[u8]) -> Option<Simple> {
        ...
        let m = generated::Simple::from_reader(&mut reader, body).ok()?;
        // --------------------------------------------------
