//! manual x rich record: hand-pushed key/length/value triples, converting each native field
...
/// [`Manual`] implementation of [`Codec`] for [`Rich`]
impl Codec<Rich> for Manual {
    fn encode(rec: &Rich) -> Vec<u8> {
        ...
        put(
            &mut out,
            key::ELAPSED,
            &(rec.elapsed.as_nanos() as u64).to_be_bytes(),
        );
        ...
        put(
            &mut out,
            key::DATE,
            &rec.date.num_days_from_ce().to_be_bytes(),
        );
        put(
            &mut out,
            key::TIME,
            &rec.time.num_seconds_from_midnight().to_be_bytes(),
        );
    ...
    fn decode(body: &[u8]) -> Option<Rich> {
        ...
        while j + 2 <= body.len() {
            ...
            match tag {
                ...
                key::TIMESTAMP => {
                    timestamp = Some(DateTime::from_timestamp_nanos(i64::from_be_bytes(
                        val.try_into().ok()?,
                    )));
                }
                key::ELAPSED => {
                    elapsed = Some(Duration::from_nanos(u64::from_be_bytes(
                        val.try_into().ok()?,
                    )));
                }
                key::ADDR => addr = Some(Ipv4Addr::from(u32::from_be_bytes(val.try_into().ok()?))),
                key::ADDR6 => {
                    addr6 = Some(Ipv6Addr::from(u128::from_be_bytes(val.try_into().ok()?)));
                }
                key::DATE => {
                    date = Some(NaiveDate::from_num_days_from_ce_opt(i32::from_be_bytes(
                        val.try_into().ok()?,
                    ))?);
                }
                key::TIME => {
                    time = Some(NaiveTime::from_num_seconds_from_midnight_opt(
                        u32::from_be_bytes(val.try_into().ok()?),
                        0,
                    )?);
                }
                key::SYMBOL => {
                    symbol = Some(char::try_from(u32::from_be_bytes(val.try_into().ok()?)).ok()?);
                }
