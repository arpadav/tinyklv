//! serde_klv x native record: serde_klv can only (de)serialize primitive/string fields, so the
//! native record is hand-mapped through a flat `NativeWire` of raw scalars (coordinate flattened
...
/// Flat raw-scalar wire form of [`NativeNested`] that serde_klv can derive.
///
...
/// hand-convert between `NativeNested` and this wire form.
...
/// [`NativeNested`] implementation of [`Serialize`]
impl Serialize for NativeNested {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        NativeWire {
            ...
            lat: self.coord.lat,
            lon: self.coord.lon,
            timestamp_nanos: self.timestamp.timestamp_nanos_opt().expect("timestamp in i64-ns range"),
...
/// [`NativeNested`] implementation of [`Deserialize`]
impl<'de> Deserialize<'de> for NativeNested {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(NativeNested {
            id: w.id,
            coord: GpsCoord { lat: w.lat, lon: w.lon },
...
/// [`SerdeKlv`] implementation of [`Codec`] for [`NativeNested`]
impl Codec<NativeNested> for SerdeKlv {
    fn encode(rec: &NativeNested) -> Vec<u8> {
        ::serde_klv::to_bytes(rec).unwrap()
    }

    fn decode(body: &[u8]) -> Option<NativeNested> {
        ::serde_klv::from_bytes::<NativeNested>(body).ok()
    }
}
