//! serde_klv x compound record: the whole reason this adapter exists.
//!
...
/// Flat wire form of [`Compound`] that serde_klv can derive
///
...
/// Both directions hand-convert between `Compound` and this wire form
...
/// [`Compound`] implementation of [`Serialize`]
impl Serialize for Compound {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        // --------------------------------------------------
        // convert Compound to flat wire form and delegate to serde_klv
        // --------------------------------------------------
        PlatformWire {
            id: self.id,
            lat: self.coord.lat,
...
/// [`Compound`] implementation of [`Deserialize`]
impl<'de> Deserialize<'de> for Compound {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        // --------------------------------------------------
        // reconstruct nested Compound from flat wire fields
        // --------------------------------------------------
        Ok(Compound {
            id: w.id,
            coord: GpsCoord {
...
/// [`SerdeKlv`] implementation of [`Codec`] for [`Compound`]
impl Codec<Compound> for SerdeKlv {
    fn encode(rec: &Compound) -> Vec<u8> {
        ::serde_klv::to_bytes(rec).unwrap()
    }

    fn decode(body: &[u8]) -> Option<Compound> {
        ::serde_klv::from_bytes::<Compound>(body).ok()
    }
}
