...
impl TryFrom<generated::Rich> for Rich {
    ...
    fn try_from(m: generated::Rich) -> Result<Self, Self::Error> {
        ...
        // --------------------------------------------------
        let timestamp =
            DateTime::<Utc>::from(SystemTime::try_from(m.timestamp.ok_or(())?).map_err(|_| ())?);
...
impl From<&Rich> for generated::Rich {
    fn from(rec: &Rich) -> Self {
        generated::Rich {
            ...
            timestamp: Some(prost_types::Timestamp::from(SystemTime::from(
                rec.timestamp,
            ))),
            elapsed: Some(
                prost_types::Duration::try_from(rec.elapsed).expect("duration in WKT range"),
            ),
...
/// [`Prost`] implementation of [`Codec`] for [`Rich`]
impl Codec<Rich> for Prost {
    fn encode(rec: &Rich) -> Vec<u8> {
        generated::Rich::from(rec).encode_to_vec()
    }

    fn decode(body: &[u8]) -> Option<Rich> {
        generated::Rich::decode(body).ok()?.try_into().ok()
    }
}
