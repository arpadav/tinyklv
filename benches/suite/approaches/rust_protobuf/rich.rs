...
impl From<&Rich> for generated::Rich {
    fn from(rec: &Rich) -> Self {
        generated::Rich {
            ...
            timestamp: protobuf::MessageField::some(PbTimestamp::from(SystemTime::from(
                rec.timestamp,
            ))),
...
/// [`RustProtobuf`] implementation of [`Codec`] for [`Rich`]
impl Codec<Rich> for RustProtobuf {
    fn encode(rec: &Rich) -> Vec<u8> {
        generated::Rich::from(rec)
            .write_to_bytes()
            .expect("write to Vec is infallible")
    }

    fn decode(body: &[u8]) -> Option<Rich> {
        generated::Rich::parse_from_bytes(body)
            .ok()?
            .try_into()
            .ok()
    }
