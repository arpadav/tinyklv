//! micropb x rich record: whole-struct conversion against the raw variant. micropb has no
//! std/chrono conversions, so the temporal fields are raw integers (the raw-scalar `Rich` schema,
//! under the `bench_native_` package module) and every native type is reconstructed by hand.
//!
//! Author: aav
// --------------------------------------------------
// local
...
impl TryFrom<bench_native_::Rich> for Rich {
    ...
    fn try_from(m: bench_native_::Rich) -> Result<Self, Self::Error> {
        // --------------------------------------------------
...
impl From<&Rich> for bench_native_::Rich {
    ...
    fn from(rec: &Rich) -> Self {
        let mut m = bench_native_::Rich::default();
        m.id = rec.id;
        m.set_coord(bench_native_::GpsCoord {
            lat: rec.coord.lat,
            lon: rec.coord.lon,
        });
        m.timestamp_nanos = rec
            .timestamp
            .timestamp_nanos_opt()
...
/// [`Micropb`] implementation of [`Codec`] for [`Rich`]
impl Codec<Rich> for Micropb {
    fn encode(rec: &Rich) -> Vec<u8> {
        let msg = bench_native_::Rich::from(rec);
