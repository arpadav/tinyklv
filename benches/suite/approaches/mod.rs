...
//!
//! Every approach directory holds an identically-named `simple.rs` / `compound.rs` pair
//! (`impl Codec<Simple>` / `impl Codec<Compound>`) plus its `mod.rs` (the unit struct
//! and its [`Approach`](crate::suite::Approach) identity). Open the eight `simple.rs` files
//! side by side to compare how each approach handles the simple record - same method names,
