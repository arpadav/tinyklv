//! tlv_parser x rich record: build a BER-TLV tree (nested coordinate sub-tree `0x22` plus a
...
/// [`TlvParser`] implementation of [`Codec`] for [`Rich`]
impl Codec<Rich> for TlvParser {
    fn encode(rec: &Rich) -> Vec<u8> {
        ...
        let children = vec![
            ...
            Tlv::new(
                0x04,
                Value::Val((rec.elapsed.as_nanos() as u64).to_be_bytes().to_vec()),
            )
            .unwrap(),
            Tlv::new(0x05, Value::Val(u32::from(rec.addr).to_be_bytes().to_vec())).unwrap(),
            Tlv::new(
                0x06,
                Value::Val(u128::from(rec.addr6).to_be_bytes().to_vec()),
            )
            .unwrap(),
            Tlv::new(
                0x07,
                Value::Val(rec.date.num_days_from_ce().to_be_bytes().to_vec()),
            )
            .unwrap(),
            Tlv::new(
                0x08,
                Value::Val(rec.time.num_seconds_from_midnight().to_be_bytes().to_vec()),
            )
            .unwrap(),
            Tlv::new(
                0x09,
                Value::Val(u32::from(rec.symbol).to_be_bytes().to_vec()),
            )
            .unwrap(),
    ...
    fn decode(body: &[u8]) -> Option<Rich> {
        ...
        Some(Rich {
            ...
            elapsed: Duration::from_nanos(u64::from_be_bytes(
                get("21 / 04")?.as_slice().try_into().ok()?,
            )),
            addr: Ipv4Addr::from(u32::from_be_bytes(
                get("21 / 05")?.as_slice().try_into().ok()?,
            )),
            addr6: Ipv6Addr::from(u128::from_be_bytes(
                get("21 / 06")?.as_slice().try_into().ok()?,
            )),
            ...
            symbol: char::try_from(u32::from_be_bytes(
                get("21 / 09")?.as_slice().try_into().ok()?,
            ))
            .ok()?,
            seq: NonZeroU32::new(u32::from_be_bytes(
                get("21 / 10")?.as_slice().try_into().ok()?,
            ))?,
