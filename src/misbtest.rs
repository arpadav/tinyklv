...
impl crate::prelude::StreamDecode<&[u8]> for Misb0601 {
    fn decode(input: &mut &[u8]) -> winnow::PResult<Self> {
        ...
        let packet_len = seq!(_:
            ...
            crate::codecs::ber::dec::ber_length,
        ...
        loop {
            match (
                crate::codecs::ber::dec::ber_oid::<u64>,
                crate::codecs::ber::dec::ber_length,
            ).parse_next(packet) {
                Ok((key, len)) => match (key, len) {
                    (0x01, _) => checksum = crate::codecs::binary::dec::be_u16(packet).ok(),
                    (0x02, _) => precision_timestamp = crate::misb::dec::precision_timestamp(packet).ok(),
                    (0x03, len) => mission_id = crate::codecs::binary::dec::to_string(packet, len).ok(),
                    (0x04, len) => platform_tail_numer = crate::codecs::binary::dec::to_string(packet, len).ok(),
                    (0x05, _) => platform_heading_angle = crate::codecs::binary::dec::be_u16(packet).ok(),
                    (0x06, _) => platform_pitch_angle = crate::codecs::binary::dec::be_i16(packet).ok(),
                    (0x07, _) => platform_roll_angle = crate::codecs::binary::dec::be_i16(packet).ok(),
                    (0x08, _) => platform_true_airspeed = crate::codecs::binary::dec::be_u8(packet).ok(),
                    (0x09, _) => platform_indicated_airspeed = crate::codecs::binary::dec::be_u8(packet).ok(),
                    (0x0a, len) => platform_designation = crate::codecs::binary::dec::to_string(packet, len).ok(),
                    (0x0b, len) => image_source_sensor = crate::codecs::binary::dec::to_string(packet, len).ok(),
                    (0x0c, len) => image_coordinate_system = crate::codecs::binary::dec::to_string(packet, len).ok(),
                    (0x0d, _) => sensor_latitude = crate::codecs::binary::dec::be_i32(packet).ok(),
                    (0x0e, _) => sensor_longitude = crate::codecs::binary::dec::be_i32(packet).ok(),
                    (0x0f, _) => sensor_true_altitude = crate::codecs::binary::dec::be_u16(packet).ok(),
                    (0x10, _) => sensor_hfov = crate::codecs::binary::dec::be_u16(packet).ok(),
                    (0x11, _) => sensor_vfov = crate::codecs::binary::dec::be_u16(packet).ok(),
                    (0x12, _) => sensor_relative_azimuth_angle = crate::codecs::binary::dec::be_u32(packet).ok(),
