...
impl KlvStructAttr {
    ...
    fn parse_struct_attr(attr: &Attribute, struct_attrs: &mut KlvStructAttr) {
        let sattr = match attr::ListedAttr::new(attr.to_token_stream().to_string()) {
            Ok(sattr) => sattr,
            Err(e) => {
...
impl Push<attr::ListedAttr> for KlvStructAttr {
    /// See [`Push::push`]
    /// 
    /// # Panics
    /// 
    ...
    fn parse_field_attr(attr: &Attribute, field_attrs: &mut KlvFieldAttr) {
        match attr.parse_meta() {
            Ok(Meta::NameValue(mnv)) => {
                let kvp = match attr::KeyValPair::try_from(mnv) {
                    Ok(kvp) => kvp,
                    Err(e) => panic!("{}", e),
                };
                field_attrs.push(kvp);
            },
            Err(_) => match attr::ListedAttr::new(attr.to_token_stream().to_string()) {
...
impl Push<attr::ListedAttr> for KlvFieldAttr {
    /// See [`Push::push`]
    /// 
    /// # Panics
    /// 
...
impl Push<attr::KeyValPair> for KlvFieldAttr {
    /// See [`Push::push`]
    /// 
    /// # Panics
    /// 
