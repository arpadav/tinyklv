...
`#[derive(Klv)]`, but they serve different callers. `DecodeValue` is the
one-shot path for an already-available body; it finalizes required fields and
surfaces truncation as an error. `DecodePartial` is the streaming path; it can
return `NeedMore(p)` with the fields decoded so far.

Typical direct use is rare - most callers reach for `Decoder` instead
(see below). Direct use is valuable when you know the input may be incomplete
and want to inspect the `Ready` / `NeedMore` distinction explicitly. If the
caller knows the body is complete after a `NeedMore(p)`, it should call
`p.finalize()`.
...
Appends this value's KLV bytes to `out` (no sentinel, no frame length).
Generated by `#[derive(Klv)]` unless `allow_unimplemented_encode` is
set. Writing into a caller-owned buffer lets one `Vec<u8>` be reused.
...
Appends a full packet to `out`: `sentinel + length + <encode_value bytes>`.
...
The literal form lowers to `key == <lit>`, so it needs an integer-comparable key
type; use the function form for any other key type.
