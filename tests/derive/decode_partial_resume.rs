...
/// Build the raw KLV body of a [`Triple`] without sentinel or outer length prefix
///
/// Encodes each field as a `key(1) + len(1) + value` triple in field order
/// Field `d` is omitted entirely when `None`.  The returned bytes are
/// suitable for direct use with [`Triple::decode_partial`] or as the inner
/// body of a manually constructed sentinel frame
...
#[test]
/// Split a framed `Triple` mid-body and feed through [`tinyklv::Decoder`]
///
/// The first half yields `None` from `next()`; completing the feed with
/// the second half produces the expected `Triple` with no residual bytes
...
#[test]
/// Truncate after the first KLV triple and verify the [`tinyklv::Decoder`] reassembles correctly
///
/// The feed cut at the key byte of `b` yields `None`; the subsequent feed
/// with the remainder yields the complete `Triple`
...
///
/// Feeds the full encoded frame in 4-byte pieces through [`tinyklv::Decoder`]
/// and asserts exactly one `Triple` emerges with `d == Some(42)`
...
/// Map a [`Packet`] result to a human-readable variant name for panic messages
///
/// Returns `"Ok(Ready)"`, `"Ok(NeedMore)"`, or `"Err(label)"` so that
/// test panics name the actual variant without requiring `Debug` on `T` or `P`
