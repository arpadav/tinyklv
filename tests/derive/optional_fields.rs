//! Optional field tests for `#[derive(Klv)]`
//!
//! Tests the `WithOptionals` struct (a required `u8` plus an optional `u16`
//! and an optional UTF-8 string). Covers: all fields present (`Some`),
//! missing optional numeric/string, both optionals absent, missing required
//! field error, full encode/decode roundtrip with all `Some` and all `None`,
//! reversed field order, duplicate required key last-wins, and empty stream
//! error
//!
//! Author: aav
// --------------------------------------------------
