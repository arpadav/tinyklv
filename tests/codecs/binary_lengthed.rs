//! Lengthed binary codec tests (`be_*_lengthed` / `le_*_lengthed`)
//!
//! Tests the variable-length binary codecs that consume exactly `len` bytes
//! and zero-pad or truncate to fit the target integer type. Covers
//! `be_u16_lengthed`, `be_u32_lengthed`, `be_u64_lengthed`,
//! `le_u16_lengthed`, `le_u32_lengthed`, and their encoder counterparts for
//! exact-size, truncation (len > native width), zero-padding (len < native
//! width), zero-length edge cases, and the doctest canonical examples
//!
//! Author: aav
#[test]
